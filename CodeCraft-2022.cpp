...
bool cmp_server_bigger(int s1, int s2) {
	return server[s1].band_limit > server[s2].band_limit;
}
...
LD how_much_this_server_cost_now(int s_id) //返回此服务器当前已经需要付多少钱
{
	...
	if (len <= percent50)return V;
	LD p95_band = server[s_id].fare_sequence[percent50].used;
...
bool cmp_server_for_pre_allocate(int i, int j) //这太怪了【也许随机打乱甚至比这个顺序还好
{
	...
	return server[i].band_limit / server[i].degree() > server[j].band_limit / server[j].degree();*/
	return server[i].band_limit > server[j].band_limit;
}
//预分配当前考虑的是哪个服务器
...
LD delta_Cost_if_add_piece_for_Center2(int s_id, int t, int p_id, LL a_piece) //t时刻如果把a_piece放入服务器s_id，将导致的中心成本至少增量
{
	LL pre_max_size = MS_t_s_p[t][s_id][p_id];
	if (pre_max_size >= a_piece)return 0;  //不会更新最大值，中心用量丝毫不增加
	LD delta = a_piece - pre_max_size;  //将导致的中心节点t时刻带宽增量
	return A * delta;
}
LD delta_Cost_if_add_piece_for_server2(int s_id, int t, LL a_piece)//t时刻如果把a_piece放入服务器s_id，将导致的成本增量，（同时考虑三段计费函数）
{
	if (server[s_id].band_limit - server[s_id].used_band[t] < a_piece)return 1e8; //根本放不下，返回极大值，表明放此服务器不可行
	if (pre_allocate_server_moment[s_id][t])return 0; //如果是在免费机会，则没有任何成本增量
	LD band_limit = server[s_id].band_limit;
	...
	LD old_cost = 0; LD new_cost = 0;
	if (server[s_id].this_server_is_used == false)old_cost = 0;
	else if (used_band_t <= V)old_cost = V;
	else old_cost = (used_band_t - V) * (used_band_t - V) * 0.95 / band_limit + used_band_t;   //【由于读入的band_limit乘过了0.95，需要在分子同时乘0.95约掉，不过其实在缓存机制下，used都是不精确的
	
	if (new_used_band_t <= V)new_cost = V;
	else new_cost = (new_used_band_t - V) * (new_used_band_t - V) * 0.95 / band_limit + new_used_band_t;
	LD ret = new_cost - old_cost;
	return ret;
}

void first_schedule()//三个for皆可排序，也可混合，比如一个时刻一个时刻地考虑，但时刻内的c_id和p_id混合考虑
{
...
void first_schedule3()  //全局排序的初次分配版本
{
...
void first_schedule4()  //全局，节点deltacost全部计入
{
	regenerate_center_fare_sequence(true); //完成预分配后，生成中心节点的当前计费序列，在初次分配时可以利用
	...
	for (int node_idx = 0; node_idx < total_piece_num; node_idx++)
	{
		...
		for (int s_idx = 0; s_idx < s_len; s_idx++)
		{
			...
			LL capacity = server[s_id].band_limit - server[s_id].used_band[t];
			if (capacity < a_piece)continue; //根本放不下，此服务器不可行
			LD server_delta = delta_Cost_if_add_piece_for_server2(s_id, t, a_piece);  //评估把小片放入s_id的代价（成本至少增量）
			LD center_delta = delta_Cost_if_add_piece_for_Center2(s_id, t, p_id, a_piece);
			LD k = 1.0; //这个k调很大也没什么变化，说明center_delta总是接近0，因为大部分的小片，都不会直接改变中心节点的95带宽，而是间接影响
			LD eva = server_delta + k * center_delta;
			LL free_capacity = server[s_id].free_space_for_server(t) - server[s_id].used_band[t];
			if (eva < minEva && fabs(minEva - eva)>1e-12)
...
void first_schedule5()  //全局，放bandlimit大的服务器
{
	regenerate_center_fare_sequence(true); //完成预分配后，生成中心节点的当前计费序列，在初次分配时可以利用
	...
	for (int node_idx = 0; node_idx < total_piece_num; node_idx++)
	{
		...
		for (int s_idx = 0; s_idx < s_len; s_idx++)
		{
			...
			LD k = 1.0; //这个k调很大也没什么变化，说明center_delta总是接近0，因为大部分的小片，都不会直接改变中心节点的95带宽，而是间接影响
			LD eva = server_delta + k * center_delta;
			LL _capacity = server[s_id].band_limit;
			if (eva < minEva && fabs(minEva - eva)>1e-12)
			{
			...
			{
				if (_capacity > max_Capacity)  //最大适配法
				{
					max_Capacity = _capacity;
					minEva = eva; selected_s = s_id;
				}
...
int now_txx;
bool cmp_xxxxx(int a, int b) {
	return server[a].used_band[now_txx - 1] > server[b].used_band[now_txx - 1];
}
	...
	for (int t = 1; t <= T; t++)
	{
		if (t > 1) {
			...
			for (int s_id = 1; s_id <= N; s_id++) {
				s_vec.push_back(s_id);
			}
			now_txx = t;
			sort(s_vec.begin(), s_vec.end(), cmp_xxxxx);
			for (int i = 0; i < 20; i++) {
				if (i == 19) {
					cout << server[s_vec[i]].name << "\n";
				}
				else {
					cout << server[s_vec[i]].name << ",";
				}
...
void read_and_copy()
{
	...
	sort(s_vec.begin(), s_vec.end(), cmp_server_bigger);
	/*
	for (int s_id = 1; s_id <= N; s_id++) {
	...
	*/
	...
	for (int s_idx = 1; s_idx <= N; s_idx++) //处理缓存机制的暂行办法
	{
		server[s_idx].band_limit *= 0.95;
...
int main()
{
	...
	//freopen("D:/资料/2022软挑/决赛/判题器/solution.txt", "w", stdout);
	...
	//percent50 = percent5;
	/*
	当A>=1.5/1.3/1.4 等等，p5该变量较小
	若P5减少量较大，则降低边缘节点成本，提高中心节点成本
	当A特大时，调用first4，减少中心节点成本
	*/
	/**/
	percent5 /=2;
	...
	//pre_allocate_all_free3();//当A较小，调用减少边缘节点成本
	first_schedule3();//
	//first_schedule2();
	//first_schedule();
	//first_schedule4();//当A较大时，可降低中心节点成本
	//first_schedule5();//当A较大，使用超级节点，

	//output_all_server_cost(); /////【调试时】，输出当前方案的成本信息

	//cout<<"16:"<<how_much_this_server_cost_now(16)<<"\n";
	//cout << "43:" << how_much_this_server_cost_now(43) << "\n";
	//cout << "93:" << how_much_this_server_cost_now(93) << "\n";
	output_ans(X_t_c_p);
}
